...
class Ui_Dialog(QtWidgets.QMainWindow):
    def setupUi(self, Dialog):
        ...
        self.gridLayoutWidget.setGeometry(QtCore.QRect(100, 60, 600, 600))
    ...
    def render_board(self):
        for x in range(8):
            for y in range(8):
                if self.grid[x][y]!='.':
                    self.buttons[(x,y)].setIcon(QIcon(f"C:\\Users\\Admin\\PycharmProjects\\chessproject\\src\\images\\{self.grid[x][y]}.png"))
    ...
    def add_buttons(self):
        ...
        for x in range(8):
            ...
            for y in range(8):
                ...
                style1,style2="background-color:rgb(119, 149, 86);border:none;","background-color:rgb(235, 236, 208);border:none;"
                if (y+cnt)%2==0:
    ...
    def show_possible_moves(self, x, y):
        ...
        # Check for checkmate
        checkmate = check_mate()[0]

        # If it's not checkmate, proceed with highlighting possible moves
        if not checkmate:
            for ex in range(8):
                for ey in range(8):
                    # Check if the square is empty
                    if self.grid[ex][ey] == '.':
                        # Check if the selected piece can make a valid move to the square
                        if movable(gr[x][y], x, y, ex, ey):
                            # Highlight the button with a green background color
                            ...
                            # If not a valid move, remove any previous icons
