...
def check(bx=-1, by=-1, wx=-1, wy=-1, mode='D'):
    ...
    # If specific king coordinates are not provided, find them in the grid
    ...
    # Check for checks to the white king (Wcheck)
    if wx >= 0:
        for x in range(8):
            for y in range(8):
                if grid[x][y] != '.':
    ...
    # Check for checks to the black king (Bcheck)
    if bx >= 0:
        for x in range(8):
            for y in range(8):
                if grid[x][y] != '.':
...
def movable(coin, x, y, ex, ey):
    ...
    if coin == 'P':
        ...
        if color == 'B':
            # Black pawn moves
            if x == 1:  # First move for black pawn
                if (ex - x == 1 or ex - x == 2) and ey == y and obstusruted(x, y, ex, ey):
                    return True
            elif ex - x == 1 and ey == y and obstusruted(x, y, ex, ey):
                return True
            elif abs(ey - y) == 1 and ex - x == 1 and grid[ex][ey][0] == 'W':
                return True
        elif color == 'W':
            # White pawn moves
            if x == 6:  # First move for white pawn
                if (x - ex == 1 or x - ex == 2) and ey == y and obstusruted(x, y, ex, ey):
                    return True
            elif x - ex == 1 and ey == y and obstusruted(x, y, ex, ey):
                return True
            elif abs(ey - y) == 1 and x - ex == 1 and grid[ex][ey][0] == 'B':
                return True
